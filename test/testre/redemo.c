...
const char *registrar = "sip:192.168.10.246";
const char *uri = "sip:test2@192.168.10.246";
...
static int auth_handler(char **user, char **pass, const char *realm, void *arg) {
	...
	err |= str_dup(user, "dccan_159");
	err |= str_dup(pass, "123456");
...
static void connect_handler(const struct sip_msg *msg, void *arg) {
	...
	//TODO
	err = sipsess_accept(&sess, sess_sock, msg, 200, "OK", name,
			"application/sdp", mb, auth_handler, NULL, false, offer_handler,
			answer_handler, establish_handler, NULL, NULL, close_handler, NULL,
			NULL);
...
int main(int argc, char *argv[]) {
	...
	err = sip_alloc(&sip, dnsc, 32, 32, 32,
			"ua demo v" VERSION " (" ARCH "/" OS ")", exit_handler, NULL);
	if (err) {
	...
	err = rtp_listen(&rtp, IPPROTO_UDP, &laddr, 10000, 30000, true, rtp_handler,
			rtcp_handler, NULL);
	if (err) {
	...
	err = sdp_media_add(&sdp_media, sdp, "audio", sa_port(rtp_local(rtp)),
			"RTP/AVP");
	if (err) {
	...
	err = sdp_format_add(NULL, sdp_media, false, "96", "PCMU", 8000, 1,
	...
	if (argc > 1) {
		...
		re_fprintf(stderr, "arv is %s\n", argv[1]);
		err = sipsess_connect(&sess, sess_sock, argv[1], name, uri, name,
		NULL, 0, "application/sdp", mb, auth_handler, NULL, false,
		...
		NULL, uri, 60, name,
		NULL, 0, 0, auth_handler, NULL, false, register_handler, NULL, NULL,
		NULL);
		if (err) {
