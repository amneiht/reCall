...
static bool sdp_cmp(struct mbuf *mb, const char *msg) {
	...
	while (pl.l && strlen(msg)) {
		...
		if (re_regex(pl.p, pl.l, "[^=]1=[^\r\n]+", &n1, &v1))
			return false;

		if (re_regex(msg, strlen(msg), "[^=]1=[^\r\n]+", &n2, &v2))
			return false;
...
int test_sdp_all(void) {
	...
	err = sdp_media_add(&audio, sess, sdp_media_audio, 5004, sdp_proto_rtpavp);
	if (err)
		goto out;

	err = sdp_format_add(NULL, audio, false, ref_pt, ref_cname, ref_srate, 1,
	NULL, NULL, NULL, false, NULL);
	err |= sdp_format_add(NULL, audio, false, "110", cname_speex, 16000, 2,
	NULL, NULL, NULL, false, NULL);
	if (err)
		goto out;
...
int test_sdp_parse(void) {
	...
	for (i = 0; i < 8; i++) {
		...
		err = sdp_format_add(NULL, audio, false, ref_pt, ref_cname, ref_srate,
		...
		const struct sa *ss = sdp_media_raddr(audio);
		fprintf(stderr, "rport is %d \n", ntohs(ss->u.in.sin_port));
	}
...
static int oa_addmedia(struct oa *oa, bool local, const char *mname,
		uint16_t port, const char *transp, enum sdp_dir dir, uint32_t ncodec,
		...) {
	struct sdp_media *m;
	va_list ap;
	int err;
	...
	err = sdp_media_add(&m, local ? oa->alice : oa->bob, mname, port, transp);
	if (err)
		return err;
...
static int rfc4317_section2_1(struct oa *oa) {
	...
	err |= oa_addmedia(oa, 1, "audio", 49170, "RTP/AVP", SDP_SENDRECV, 3, "0",
			"PCMU", 8000, "8", "PCMA", 8000, "97", "iLBC", 8000);
	err |= oa_addmedia(oa, 1, "video", 51372, "RTP/AVP", SDP_SENDRECV, 2, "31",
			"H261", 90000, "32", "MPV", 90000);
	err |= oa_addmedia(oa, 0, "audio", 49174, "RTP/AVP", SDP_SENDRECV, 1, "0",
			"PCMU", 8000);
	err |= oa_addmedia(oa, 0, "video", 49170, "RTP/AVP", SDP_SENDRECV, 1, "32",
			"MPV", 90000);
	if (err)
		return err;

	err = oa_offeranswer(oa, "v=0\r\n"
...
static int rfc4317_section2_2(struct oa *oa) {
	...
	err |= oa_addmedia(oa, 1, "video", 51372, "RTP/AVP", SDP_SENDRECV, 2, "31",
			"H261", 90000, "32", "MPV", 90000);
	err |= oa_addmedia(oa, 0, "audio", 49172, "RTP/AVP", SDP_SENDRECV, 2, "0",
			"PCMU", 8000, "8", "PCMA", 8000);
	if (err)
		return err;

	return oa_offeranswer(oa, "v=0\r\n"
			"o=alice 2890844526 2890844526 IN IP4 1.2.3.4\r\n"
			"s=-\r\n"
			"c=IN IP4 1.2.3.4\r\n"
			"t=0 0\r\n"
...
static int rfc4317_section2_4(struct oa *oa) {
	...
	err |= oa_addmedia(oa, 1, "audio", 49170, "RTP/AVP", SDP_SENDRECV, 2, "0",
			"PCMU", 8000, "97", "iLBC", 8000);
	err |= oa_addmedia(oa, 1, "audio", 49172, "RTP/AVP", SDP_SENDRECV, 1, "98",
			"telephone-event", 8000);
	err |= oa_addmedia(oa, 0, "audio", 49172, "RTP/AVP", SDP_SENDRECV, 1, "97",
			"iLBC", 8000);
	err |= oa_addmedia(oa, 0, "audio", 49174, "RTP/AVP", SDP_RECVONLY, 1, "98",
			"telephone-event", 8000);
	if (err)
		return err;
...
int test_sdp_bfcp(void) {
	static const char *msg_offer = "v=0\r\n"
			"o=alice 2890844526 2890844526 IN IP4 1.2.3.4\r\n"
			"s=-\r\n"
			"c=IN IP4 1.2.3.4\r\n"
			"t=0 0\r\n"
...
int test_sdp_extmap(void) {
	...
	TEST_STRCMP("urn:ietf:params:rtp-hdrext:ssrc-audio-level",
			strlen("urn:ietf:params:rtp-hdrext:ssrc-audio-level"), ext.name.p,
			ext.name.l);
	...
	TEST_STRCMP("http://example.com/ext.htm#xmeta",
			strlen("http://example.com/ext.htm#xmeta"), ext.name.p, ext.name.l);
	TEST_STRCMP("short", strlen("short"), ext.attrs.p, ext.attrs.l);
	TEST_EQUALS(SDP_SENDRECV, ext.dir);
	TEST_ASSERT(ext.dir_set);
	...
	TEST_EQUALS(0, err);
	TEST_STRCMP("URI-gps-string", strlen("URI-gps-string"), ext.name.p,
			ext.name.l);
	TEST_ASSERT(!pl_isset(&ext.attrs));
