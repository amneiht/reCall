...
static bool sdp_cmp(struct mbuf *mb, const char *msg) {
	...
	while (pl.l && strlen(msg)) {
		...
		if (0 != pl_cmp(&v1, &v2)) {
			log_warn("value mismatch: %r=%r\n", &n1, &v1);
			return false;
		}
	}

	if (pl.l) {
		log_warn("%u bytes junk at end: %r\n", pl.l, &pl);
	}

	if (strlen(msg)) {
		log_warn("%u bytes junk at end: %s\n", strlen(msg), msg);
	}
...
int test_sdp_all(void) {
	...
	if (!sc) {
		log_warn("codec not found: speex\n");
		err = ENOENT;
		goto out;
	}
	...
	if (sc) {
		log_warn("unexpected codec found\n");
		err = EINVAL;
		goto out;
	}
...
int test_sdp_parse(void) {
	...
	for (i = 0; i < 6; i++) {
		...
		fprintf(stderr, "port is %d\n", ntohs(sl->u.in.sin_port));
	}
	fprintf(stderr, "no err");
	out: mem_deref(sess);
	mem_deref(mb);
